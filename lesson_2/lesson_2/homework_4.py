...
def validate_inn(inn_str: str) -> bool:
    ...
    """
    # Шаг 1: Очищаем строку от возможных пробелов
    inn_str = inn_str.strip()
    
    # Шаг 2: Проверяем, что строка состоит только из цифр
    if not inn_str.isdigit():
        return False
    
    # Шаг 3: Проверяем длину ИНН (10 или 12 цифр)
    if len(inn_str) == 10:
...
def validate_inn_legal_entity(inn: str) -> bool:
    ...
    """
    # Весовые коэффициенты для 10-значного ИНН (для первых 9 цифр)
    coefficients = [2, 4, 10, 3, 5, 9, 4, 6, 8]
    
    # Вычисляем контрольную сумму для первых 9 цифр
    control_sum = 0
    for i in range(9):
...
def validate_inn_individual(inn: str) -> bool:
    ...
    Алгоритм описан в задании.
    """
    # Первый этап: коэффициенты для первых 10 цифр
    ...
    # Вычисляем первое контрольное число (11-я цифра)
    check_digit_1 = control_sum_1 % 11
    if check_digit_1 > 9:
        check_digit_1 %= 10
    
    # Сравниваем с 11-й цифрой
    if check_digit_1 != int(inn[10]):
        return False
    
    # Второй этап: коэффициенты для первых 11 цифр
    ...
    # Вычисляем второе контрольное число (12-я цифра)
...
# ========== ПРИМЕРЫ ИСПОЛЬЗОВАНИЯ (для самопроверки) ==========
if __name__ == "__main__":
    # Валидные ИНН для теста (реальные существующие номера)
    test_inns = [
        ("7712345678", True),  # пример 10-значного ИНН (юридическое лицо)
    ...
    # Дополнительно: проверка реальных ИНН организаций (если знаете)
    # real_inn = "7707083893"  # пример ИНН Сбербанка (проверьте сами)
    # print(f"\nРеальный ИНН: {real_inn} -> {validate_inn(real_inn)}")
